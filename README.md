...
主题共享实现位于 `themes/beamerthemeVSP.sty`。`\VSPsetspeaker[Speaker]{姓名}{详情}` 统一设置封面的标签、姓名和详情，同时同步标准 Beamer `\author`/`\institute` 元数据；直接使用 `\author` 和 `\institute` 仍然兼容。通过仓库 Makefile 构建时会自动配置 `TEXINPUTS`，文稿中不需要硬编码主题绝对路径。
