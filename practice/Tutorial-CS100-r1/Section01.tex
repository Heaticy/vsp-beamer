...
\begin{frame}
    ...
    \end{vspquote}
    Windows上编程，往往要先请出一个庞大的\textbf{IDE}（Integrated Design Environment）
    \begin{itemize}
...
\begin{frame}
    ...
    \begin{itemize}
        \item \textbf{WSL}（Windows Subsystem for Linux）：适用于Windows的Linux子系统，是微软官方提供的功能，本质上可以认为是一个在Windows上运行但\textbf{只有命令行环境的Linux虚拟机}。该方案适用于想继续使用Windows作为主力系统，只是想获得一个Linux开发环境的学生。
        \item \textbf{双系统}：你可以在磁盘上划出一块空间，在保留当且Window系统的情况安装完整的Linux系统，开机时可以选择启动哪个系统。该方案适用于希望完整体验Linux桌面环境的学生。
        \item 推荐使用Ubuntu 26.04或Ubuntu 24.04，但你也可以使用其他Linux发行版。
        \item 此外，对于MacOS用户，你可以直接在MacOS上进行开发，无需Linux虚拟机！
    \end{itemize}
    我们真诚鼓励所有学生尝试双系统方案\footnote{HEAD TA承诺为在安装双系统的过程中划遇到技术问题的学生提供技术支持。}，体验在一个完整的Linux桌面系统上工作。
\end{frame}

\begin{frame}
    \frametitle{WSL：最简单的开始方式}
    WSL的安装需要在\textbf{正确的网络环境}下，在PowerShell上执行
    \begin{Code}*
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item \textbf{创建分区会格式化对应区域，请务必不要格式化Windows正在使用的分区！}
