...
\begin{itemize}
\item
  \VSPstrong{Make}: 一个自动化构建工具，通过读取 Makefile 来决定如何编译程序。
\item
  \VSPstrong{Makefile}: Make 的配置文件（默认名称 \texttt{Makefile}），描述了文件之间的依赖关系。
\end{itemize}
...
\footnotesize
\VSPstrong{Via Package Manager (Ubuntu)} 通常包含在 \texttt{build-essential} 中。

\begin{lstlisting}[language=bash]
sudo apt install make
\end{lstlisting}

\VSPstrong{Compiling from Source (源码编译)} 为什么？Make 4.4+ 对 \texttt{.NOTPARALLEL} 支持更好。Ubuntu 24.04 默认是 4.3。
...
\VSPstrong{问题} 1. \VSPstrong{繁琐}：每次都要敲长命令。(可以通过写成脚本来解决) 2. \VSPstrong{低效}：即使只改了 \texttt{main.cpp}，\texttt{utils.cpp} 也会被重新编译（全量编译）。
\end{frame}
...
\item
  \VSPstrong{Target 不存在}：执行命令。
\item
  \VSPstrong{Target 存在}：检查 \VSPstrong{时间戳 (Timestamp)}。

  \begin{itemize}
  \item
    如果任何一个 \VSPstrong{Dependency} 比 \VSPstrong{Target} 新 -\textgreater{} \VSPstrong{重新编译}。
  \item
    如果 \VSPstrong{Target} 比所有 \VSPstrong{Dependency} 都新 -\textgreater{} \VSPstrong{跳过}。
  \end{itemize}
\end{enumerate}
...
\begin{itemize}
\item
  \VSPstrong{定义}: \texttt{VAR\ :=\ value}
\item
  \VSPstrong{引用}: \texttt{\$(VAR)} 或 \texttt{\$\{VAR\}}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item
  \VSPstrong{\texttt{:=} (Simple/Immediate)}（\VSPstrong{推荐默认使用}）:

  \begin{itemize}
  \item
    \VSPstrong{立即展开}。定义时就确定值。类似 C++ 的``传值''。
  \end{itemize}
\item
  \VSPstrong{\texttt{=} (Recursive)}:

  \begin{itemize}
  \item
    \VSPstrong{延后展开}。使用时才确定值。类似 C++ 的``传引用''。
  \end{itemize}
...
\end{lstlisting}
\item
  \VSPstrong{\texttt{?=} (Conditional)}:

  \begin{itemize}
  \item
    如果变量\VSPstrong{未定义}，则赋值。常用于允许用户通过命令行覆盖配置。
  \item
    非常适合作为参数的默认值设置！
  \end{itemize}
\end{itemize}
...
\VSPstrong{Pattern Rule (模式规则)} 使用 \texttt{\%} 通配符：
...
\VSPstrong{Automatic Variables (自动化变量)} - \VSPstrong{\texttt{\$@}}: \VSPstrong{Target} (目标文件) - \VSPstrong{\texttt{\$\textless{}}}: \VSPstrong{First Dependency} (第一个依赖) - \VSPstrong{\texttt{\$\^{}}}: \VSPstrong{All Dependencies} (所有依赖)
\end{frame}
...
\VSPstrong{常用函数} - \VSPstrong{\texttt{wildcard}}: 扫描匹配的文件。 \texttt{makefile\ \ \ \ \ SRC\ :=\ \$(wildcard\ *.cpp)\ \ \#\ 结果:\ main.cpp\ utils.cpp} - \VSPstrong{\texttt{patsubst}}: 模式替换。 \texttt{makefile\ \ \ \ \ OBJ\ :=\ \$(patsubst\ \%.cpp,\ \%.o,\ \$(SRC))\ \#\ 结果:\ main.o\ utils.o}
\end{frame}
...
\footnotesize
\VSPstrong{场景}: 你修改了 \texttt{utils.h}，但没有修改 \texttt{main.cpp}。 \VSPstrong{结果}: Make 认为 \texttt{main.o} 是最新的，\VSPstrong{不会重新编译}。 \VSPstrong{原因}: Makefile 只知道 \texttt{main.o:\ main.cpp}，不知道它还依赖 \texttt{utils.h}。

我们需要让 Make \VSPstrong{自动学会}这些依赖关系！
\end{frame}

\begin{frame}[fragile,shrink=12]{Solution: Automatic Dependency Generation}
\footnotesize
\VSPstrong{原理}: 1. \VSPstrong{编译期}: 编译器 (\texttt{gcc/g++}) 分析 \texttt{\#include}，生成依赖文件 (\texttt{.d})。 2. \VSPstrong{读取期}: Makefile 使用 \texttt{-include} 指令读取这些 \texttt{.d} 文件。

\VSPstrong{关键 Flags}: - \VSPstrong{\texttt{-MMD}}: 生成依赖文件 (忽略系统头文件)。 - \VSPstrong{\texttt{-MP}}: 为头文件生成伪目标 (防止删除头文件后报错)。
\end{frame}
...
Makefile 执行 \texttt{-include\ main.d} 后，内存中增加了上面的规则。 现在，\VSPstrong{修改 \texttt{utils.h} 会触发 \texttt{main.o} 重编！}
\end{frame}
...
\VSPstrong{使用}: \texttt{make\ DEBUG=0} (构建 Release 版)
\end{frame}
...
\begin{itemize}
\item
  \VSPstrong{\texttt{@}}: \VSPstrong{静默执行}。只输出命令结果，不打印命令本身。
\item
  \VSPstrong{\texttt{-}}: \VSPstrong{忽略错误}。即使命令失败（如删除不存在的文件），也继续执行。
\end{itemize}
